...
public abstract class AbstractAndroidDevice {
	...
	public void launchActivity(String packageName, String activityName, String deviceSerial, String host, String user,
			String password) throws Exception {
		WindowsDefaultCliConnection windowsConsole = new WindowsDefaultCliConnection(host, user, password);// not
																											// good
		if (device.getState() == IDevice.DeviceState.ONLINE) {
			logger.info("call adb Command to bring another activity to the front activity name : " + activityName
					+ " Main Packege" + packageName + " \n");
			if (deviceSerial == null) {
				logger.info("Failed to set into front the requested activity , MobileCliConnection is not initialized");
				throw new Exception(
						"Failed to set into front the requested activity, MobileCliConnection is not initialized");
				...
				try {
					...
					// android.intent.action.MAIN -n
					// il.co.topq.mobile.server.application/.RobotiumServerActivity");
					CliCommand cmd = new CliCommand("adb -s " + deviceSerial
							+ " shell am start -a android.intent.action.MAIN -n " + packageName + "/." + activityName);// +
																														// " --activity-brought-to-front"
																														// +
																														// " \n"
					windowsConsole.handleCliCommand(
							"activity-brought-to-front activity name : cmd String is : " + cmd.getCommands()[0], cmd);
				} catch (Exception e) {
			...
			// String.format("adb -s %s shell am start -a android.intent.action.MAIN -n %s/.%s",
			// deviceSerial, packageName,activityName);//
			// //"adb shell am start -a android.intent.action.MAIN -n il.co.topq.mobile.server.application/.RobotiumServerActivity";
			// device.executeShellCommand(command,new IShellOutputReceiver() {
			//
			// @Override
